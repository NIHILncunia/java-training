...
public class tr18Methods {
  public static void main(String[] args) {
    ...
    // protected 의 경우에는 같은 패키지 내에서 기본적으로 접근이 가능하고, 다른 패키지라면 상속을 받은 클래스에 한해서 사용할 수 있게 된다.
...
// 메소드 안에서 변수가 만들어지면 그 변수는 메소드 밖에서는 사용할 수 없다. 메소드가 실행될 때에 선언되고, 소멸한다.
// 메소드 밖에 만들어진 변수는 클래스 내에서 모든 변수나 메소드가 사용할 수 있게 된다.
// 여기에 static 까지 붙게 된다면 다른 클래스에서도 사용이 가능해지는 것이다.
// 자바에는 지역변수 전역변수 같은 개념이 없다고 하지만 메소드 안에 있는 것은 지역변수, 밖에 있는 것은 전역 변수라고 할 수 있겠다.

// 또한 이 변수들을 필드라고 부른다.

// 전역 변수는 두가지가 있게 된다. 하나는 클래스 변수고 다른 하나는 인스턴스 변수이다.
// 클래스 변수는 static 이 붙어있는 변수를 말하고 인스턴스 변수는 없는 변수를 말한다.
// 클래스 변수는 프로그램이 꺼질 때에 효력을 다하게 되고, 인스턴스 변수는 인스턴스가 사라질 때에 효력을 다하게 된다.
// 지역 변수는 메소드가 끝나면 사라진다. 위에서 이미 언급했다.